            ...
            <el-form :model="form">
                <el-form-item label="订单状态" label-width="120px">
                    <el-select v-model="form.state" placeholder="请选择订单状态">
                        ...
                        <el-option label="已支付，待发货" :value="2"></el-option>
                        <el-option label="已发货，待收货" :value="3"></el-option>
...
<script>
    ...
    export default {
        ...
        methods:{
            saveOrder(){
                let _this = this;
                _this.$ajax.post(process.env.VUE_APP_SERVER + "/admin/order/edit-state", _this.form).then((response)=>{
            ...
            removeOrder(){
                let _this = this;
                _this.$ajax.post(process.env.VUE_APP_SERVER + "/admin/order/remove", _this.form).then((response)=>{
                    let resp = response.data;
                    if(resp.code === 0){
                        _this.$message.success(resp.msg);
                        _this.getOrderList();
            ...
            getOrderList(){
                ...
                _this.$ajax.post(process.env.VUE_APP_SERVER + "/admin/order/list", data).then((response)=>{
                    let resp = response.data;
                    if(resp.code === 0){
            ...
            openView(){
                let _this = this;
                if(_this.multipleSelection.length !== 1){
                    _this.$message.warning("请选择一条数据进行查看详情！");
                    return false;
                }
                _this.$ajax.post(process.env.VUE_APP_SERVER + "/admin/order/order-item", {id:_this.multipleSelection[0].id}).then((response)=>{
                    let resp = response.data;
                    if(resp.code === 0){
