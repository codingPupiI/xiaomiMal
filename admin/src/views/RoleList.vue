<template>
    <div id="role-list">
        <div style="margin-top: 20px">
            ...
            <el-input style="width:200px;height:50px;padding-left: 20px;" v-model="searchContent" placeholder="输入角色名称搜索..."/>
            <el-button style="margin-left: 5px;" type="primary" @click="getRoleList">搜索</el-button>
        </div>
        <el-table
                ref="multipleTable"
                :data="tableData"
...
<script>
    export default {
        ...
        methods: {
            ...
            removeRole(){
                let _this = this;
                _this.$ajax.post(process.env.VUE_APP_SERVER + "/admin/role/remove", _this.form).then((response)=>{
            ...
            saveRole(){
                let _this = this;
                _this.$ajax.post(process.env.VUE_APP_SERVER + "/admin/role/save", _this.form).then((response)=>{
                    let resp = response.data;
                    if(resp.code === 0){
            ...
            getRoleList(){
                ...
                _this.$ajax.post(process.env.VUE_APP_SERVER + "/admin/role/list", data).then((response)=>{
                    let resp = response.data;
                    if(resp.code === 0){
